...
void read_report_usage(void)
{
	printf("\t{report_type} [-n {number of readings}] [-c]\n");
...
{
	printf("Version %s\n", VERSION);
	printf("Usage: %s {report_type} [-n {number of readings}] [-c]\n", name);
	printf("\t[-n] - number of report readings to take\n");
	printf("\t[-c] - display output in 2D format\n");
...
#ifdef SYNA_TOOL_BOX
int read_report_main(int argc, char* argv[])
#else
